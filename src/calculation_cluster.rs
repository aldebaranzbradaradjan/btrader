...
impl CalculationCluster {
    pub fn new(
        ...
        let config_clone = config.clone();
        let bot: DiscordBot = DiscordBot::new(config_clone);
    ...
    pub fn start(&self) {
            ...
            || self.config.trading_execution_cap == -1
        {
            for rel in &relationships_names {
                // println!("------------------------------------------------------------------------------------------------");
                let deal =
                    self.calculate_relationship(relationships.get(&rel.clone()).unwrap().clone());
...
impl Deal {
    ...
    pub fn add_action(
        ...
        self.actions
            .push(Action::new(pair, action, quantity, last_price))
    }
